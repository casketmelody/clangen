...
class DebugScreen(Screens):
    ...
    def handle_event(self, event):
        if event.type == pygame_gui.UI_BUTTON_START_PRESS:
            ...
            elif event.ui_element == self.kittypet:
                Cat.become_kittypet(self.the_cat)
                self.update_selected_cat()
            elif event.ui_element == self.loner:
                Cat.become_loner(self.the_cat)
                self.update_selected_cat()
            elif event.ui_element == self.join:
                Cat.add_to_clan(self.the_cat)
                self.update_selected_cat()
            elif event.ui_element == self.lost:
                Cat.become_lost(self.the_cat)
                self.update_selected_cat()
    ...
    def screen_switches(self):
        ...
        # Column 1
        self.kittypet = UISurfaceImageButton(
        ...
        self.loner = UISurfaceImageButton(
            ui_scale(pygame.Rect((48, 0), (172, 36))),
            "become loner",
            get_button_dict(ButtonStyles.LADDER_MIDDLE, (172, 36)),
            object_id="@buttonstyles_ladder_middle",
            anchors={"top_target": self.kittypet},
        )
        self.join = UISurfaceImageButton(
            ui_scale(pygame.Rect((48, 0), (172, 36))),
            "join clan",
            get_button_dict(ButtonStyles.LADDER_MIDDLE, (172, 36)),
            object_id="@buttonstyles_ladder_middle",
            anchors={"top_target": self.loner},
        )
        self.lost = UISurfaceImageButton(
            ui_scale(pygame.Rect((48, 0), (172, 36))),
            "get lost",
            get_button_dict(ButtonStyles.LADDER_MIDDLE, (172, 36)),
            object_id="@buttonstyles_ladder_middle",
            anchors={"top_target": self.join},
        )
    ...
    def update_disabled_buttons(self):
        ...
        # self.loner.disable()
        # self.kittypet.disable()
        # self.join.disable()
        # self.lost.disable()

        if (
            self.the_cat.status.rank == CatRank.LEADER
            or self.the_cat.status.rank == CatRank.DEPUTY
        ):
            self.loner.disable()
            self.kittypet.disable()
            self.join.disable()
